...
export default function StockageEntre({ onSuccess }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:8888/projet de stage/api/stock.php', dataToSend);

            console.log('Réponse du serveur:', response.data);

            if (response.data.status === 'success') {
                alert(response.data.message || 'Entrée enregistrée avec succès !');
                setFormData({
                    date_stockage: '',
                    lot_number: '',
                    categorie: '',
