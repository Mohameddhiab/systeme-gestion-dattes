...
export default function TableChambre2() {
    ...
    const fetchData = useCallback(async () => {
        try {
            setLoading(true);
            const response = await axios.get(`http://localhost:8888/projet de stage/api/chambre2.php?search=${searchQuery}`);
    ...
    const handleDelete = async (id, type) => {
        if (window.confirm("Voulez-vous vraiment supprimer cet enregistrement ?")) {
            try {
                const response = await axios.delete(`http://localhost:8888/projet de stage/api/chambre2.php?id=${id}&type=${type}`);
                if (response.data.status === "success") {
                    fetchData();
                } else {
    ...
    return (
        <div className="glass-panel p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-gray-800">Stock Chambre 2</h2>
